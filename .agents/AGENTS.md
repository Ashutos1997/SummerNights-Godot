...
**CRITICAL CONCISENESS RULE:** Keep all 4 core documentation files (`docs/features.md`, `docs/changelog.md`, `docs/ui_design_system.md`, and `docs/HUD_UI.md`) concise, punchy, and easily readable. Avoid overly wordy descriptions, purple prose, and redundant exposition. State mechanics, design parameters, and release notes clearly, directly, and compactly.
...
**ITCH.IO EXPORT RULE:** When exporting the game for itch.io, you MUST use Godot export feature tags (e.g., `safe_audio`) to swap out any copyrighted audio (like the ripped Kamen Rider Catastrom ultimate audio) with a safe, royalty-free alternative. The ripped audio should only be kept for the GitHub `main` branch. Additionally, ensure the exported files are distinctly named to avoid confusion (e.g., `SummerNights-GithubVersion` for the default GitHub build, and `SummerNights-ItchVersion` for the safe audio build).
