...
* **PauseScreen (`ESC` or Auto-Pause):** Freezes the 3D scene tree (`get_tree().paused = true`). Includes Settings, Filters, Controls, Credits, Achievements, and Buffs. Features a broken-border design with an animated sun. The Credits screen features an autoscrolling bilingual listing with complete 1:1 third-party asset attributions (including Godot Engine branding, procedural boot splash, coronal halo, and stats plates), audio/visual systems, and non-profit fan project disclaimers matching README documentation.
* **Achievements Screen:** 3-column retro list showing status readouts across all 12 achievements inside an autoscrolling container. Locked achievements display live numerical counters (`current / max`) and mini gold progress bars (8px, 4px radii); completed achievements display a cyber gold completion badge.
...
* **End State Screens:** Win, End, and Lose screens. Lose screen is centered to emphasize the Supernova cinematic. In Endless Mode, it tracks and persists `best_wave` alongside survival time, showing celebratory `(NEW BEST!)` / `(최고 기록!)` badges when new milestones are achieved.
