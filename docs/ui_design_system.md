...
*   **Retro Post-Processing Filters (Mutually Exclusive):**
    *   **Retro Colors:** Simulates authentic PlayStation 1 / Sega Saturn 15-bit color space (32 discrete levels per channel = 32,768 colors). Replicates analog composite video characteristics via horizontal NTSC chroma delay/bleed (`SCREEN_PIXEL_SIZE.x * 1.5`), paired with a subtle 3.5% CRT aperture grille micro-scanline modulation across alternating pixel lines.
    *   **Dithering:** High-precision 8x8 Bayer matrix (64 threshold levels) replacing coarse 4x4 matrixes to eliminate contour banding. Features resolution-independent virtual retro grid scaling (`SCREEN_PIXEL_SIZE.y * 540.0`) so dither dot chunkiness is identical across 720p, 1080p, 1440p, and 4K displays, modulated by a luma-weighted curve (`clamp(1.2 - luma * 0.5, 0.35, 1.15)`) to preserve clean specular highlights while stippling midtones and sunset gradients.
    *   **Heatwave 1984:** Vintage 35mm / Kodachrome film simulation utilizing a 4-tap radial cross optical halation diffusion kernel in screen UV (`SCREEN_PIXEL_SIZE * 5.0`), scattering rich amber-red light around the Sun's perimeter and specular flares into surrounding pixels. Uses a film tone shoulder compression curve ($C / (1.0 + 0.22C)$) to prevent harsh highlight clipping and preserve facial expressions, coupled with vintage anamorphic lens falloff and celluloid grain.
