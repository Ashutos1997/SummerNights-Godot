...
* **Settings Categorized Section Badges & Row Typography:** Tactile retro plate badges (`4px` corner radii, `Color(1.0, 0.85, 0.2, 0.12)` fill, `1px` gold border `Color(1.0, 0.85, 0.2, 0.5)`, margins 10px L/R, 2px T/B) with Kenney Future 13px (EN) / Galmuri11 14px (KR) gold text (`Color(1.0, 0.85, 0.2, 1.0)`), accompanied by a 1px trailing golden hairline rule (`Color(1.0, 0.85, 0.2, 0.28)`) with 12px separation, grouping settings rows into Audio, Gameplay & Controls, and Display & System categories with balanced section spacing (36px clearance, 26px title/footer gap, 16px row spacing). Setting row labels utilize crisp off-white body text (`Color(0.92, 0.92, 0.92, 0.95)`) with 2px black outlines to provide clear visual contrast against gold category plates and interactive controls.
