...
*   **Post-Match Statistics Panel:** When a wave is cleared, the win screen now displays a staggered animated stat breakdown showing Max Combo, Water Used, Flares Caught, and Clear Time. Values tick up from 0 to their final number with a smooth counter animation. Each row fades in sequentially. Fully localized in EN/KR. The win screen auto-extends to 4.5s to allow time to read the stats.
