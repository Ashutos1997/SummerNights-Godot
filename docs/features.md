...
* **Available Weapons:**
  ...
  * *Kitsune Buster IX:* High-tier celestial buster with translucent revolving Kyubi drum, 3.0x crit multiplier, and dual-blade frame.
...
* **Celestial Awakening (Nine Tails):** 15-second super state exclusive to the Kitsune Buster IX. Generates 9 procedural cyan/white hydro-ribbon tails cascading from an overhead celestial canopy (staggered bloom, traveling waves, 3D twist, aim inertia). While active, transforms water fire into a 9-stream converging helical hydro-cannon with infinite capacity (tank locked at 100%), 2.0x cooling power, and a 5.5m Creation Aura that vaporizes incoming solar flares with `+REWRITE!` combat feedback. Swapping away from the Kitsune Buster immediately deactivates the awakening.
