...
### Fixed
* **Start Screen UI Filter Isolation:** Fixed an issue where enabling any retro post-processing filter (Retro Colors, Dithering, PS1 Shading, Heatwave 1984) and quitting back to the title screen caused the filter to bleed onto the start screen UI. Wrapped the Title Screen in a dedicated `CanvasLayer` at `layer = 10` (matching the in-game HUD layer), ensuring all title typography, start buttons, language toggles, and modal dialogues remain crisp and unaffected by the layer 0 post-processing shader.
...
* **Heat Bar Reset & Transition Lerp Fix:** Fixed a bug where starting Level 2+ or Wave 2+ displayed 99°C instead of 100°C. `HeatBar`'s step was defaulting to `1.0` in Godot, truncating floating-point lerp calculations at 98.0/99.0, and wave/level resets did not immediately snap the bar value from empty to full. Configured `step = 0.0` for continuous sub-pixel interpolation, added a snap threshold on resets in `_on_heat_changed()`, and explicitly re-emitted `heat_changed` upon wave reload.
* **Wave Timer Pulse Tween Leak:** Fixed a bug where reaching the final 10 seconds of a wave permanently started an infinite alpha-pulsing tween that never cleared, persisting across subsequent wave transitions and level reloads. Added `_stop_timer_pulse()` to rigorously clean up and terminate the tween upon wave resets, level starts, or when time is above 10 seconds.
...
* **Solar Flare Impact Penalties:** Failing to intercept a Solar Flare now severely disrupts gameplay flow. On top of the existing screen shake, a flare impact instantly resets the active Combo multiplier to zero. Additionally, both the heat damage and water tank drain now scale dynamically based on the current Wave/Level. The heat penalty scales from `4.0°C` up to a maximum cap of `12.0°C`, while the water drain scales from `10%` up to a `20%` capacity vaporization penalty.
...
### Fixed
* **HUD Right Alignment:** Fixed a UI layout issue where the Time, Score, and resource meters (Water, Ice, Catastrom) were slightly misaligned on the right side of the screen. Timer and Score are now grouped in a unified container, and resource row layouts have been standardized to ensure perfect right-alignment.
...
* **열기 게이지 리셋 및 전환 보간 오류 수정 (Heat Bar Reset & Transition Lerp Fix):** 레벨 2 이상 또는 웨이브 2 이상의 스테이지가 시작될 때 온도가 100°C가 아닌 99°C로 표시되던 버그를 수정했습니다. Godot의 `TextureProgressBar` 기본 `step`이 `1.0`으로 설정되어 있어 부동소수점 선형 보간(lerp) 계산이 98.0/99.0 정수 경계에서 잘리던 문제를 해결하기 위해 `step = 0.0`으로 설정했습니다. 또한 웨이브 및 레벨 전환 시 빈 상태에서 100%로 게이지를 즉시 동기화하도록 `_on_heat_changed()`에 스냅 처리를 추가하고 레벨 리로드 시 `heat_changed` 신호를 즉시 재발행하도록 개선했습니다.
* **웨이브 타이머 펄스 트윈 누수 수정 (Wave Timer Pulse Tween Leak):** 웨이브 타이머가 마지막 10초 이하로 떨어졌을 때 실행되는 알파 펄스 트윈이 웨이브 전환이나 레벨 재시작 후에도 종료되지 않고 영구 지속되던 버그를 수정했습니다. 웨이브 재설정, 레벨 시작 또는 10초 초과 시 트윈을 완전히 종료하고 텍스트 알파 및 스케일을 원래대로 복원하는 `_stop_timer_pulse()` 정리 로직을 추가했습니다.
...
* **태양 플레어 충돌 페널티 (Solar Flare Impact Penalties):** 태양 플레어 요격에 실패하면 이제 게임 플레이 흐름에 심각한 지장을 줍니다. 기존의 화면 흔들림 외에도, 플레어 충돌 시 즉시 활성 콤보 배수가 0으로 초기화됩니다. 또한 열 피해와 물탱크 증발량 모두 현재 웨이브/레벨에 따라 동적으로 스케일링됩니다. 열 페널티는 `4.0°C`에서 최대 `12.0°C`까지 확장되며, 물 증발 페널티는 `10%`에서 최대 `20%`까지 확장됩니다.
...
### 수정됨
* **HUD 우측 정렬 버그 수정 (HUD Right Alignment):** 시간, 점수, 그리고 자원 게이지(물, 얼음 폭발, 카타스트롬)가 화면 우측에서 미세하게 어긋나 보이던 UI 레이아웃 문제를 수정했습니다. 타이머와 점수는 통합 컨테이너로 그룹화되었으며, 자원 행 레이아웃이 표준화되어 완벽한 우측 정렬을 보장합니다.
