...
### Improved
* **Retro Post-Processing Filters Overhaul:** Substantially elevated the visual fidelity and shader mathematics of all 3 in-game retro filters:
  * **Retro Colors:** Upgraded quantization to authentic PlayStation 1 / Sega Saturn 15-bit color space (32 levels per channel = 32,768 discrete colors), eliminating coarse banding noise across sand and foliage. Added analog NTSC composite horizontal chroma delay/bleed (`SCREEN_PIXEL_SIZE.x * 1.5`) and subtle 3.5% CRT aperture grille micro-scanline modulation.
  * **Dithering:** Upgraded from a coarse 4x4 matrix to a high-precision 8x8 Bayer matrix (64 threshold levels) for silky cross-hatching without contour banding. Implemented resolution-independent virtual retro grid scaling (`SCREEN_PIXEL_SIZE.y * 540.0`) so dither dot scale remains identical across 720p, 1080p, 1440p, and 4K displays, paired with luma-weighted threshold modulation to preserve clean specular highlights.
  * **Heatwave 1984:** Replaced single-pixel math with true multi-tap optical halation diffusion (4-tap radial cross kernel in screen UV) that realistically scatters warm amber-red light around the Sun's silhouette and intense flares into adjacent pixels. Implemented an analog photographic film tone shoulder compression curve ($C / (1.0 + 0.22C)$) preserving the Sun's facial expressions and cloud volume without harsh clipping, alongside vintage anamorphic lens falloff and organic celluloid grain.
...
### 개선됨
* **레트로 후처리 필터 전면 개편 (Retro Post-Processing Filters Overhaul):** 게임 내 3종의 레트로 시각 필터의 셰이더 수학 및 렌더링 품질을 대폭 향상했습니다:
  * **레트로 색상 (Retro Colors):** 플레이스테이션 1 및 세가 새턴의 고유 15비트 색 공간(RGB 채널당 32단계 = 총 32,768색)으로 양자화를 업그레이드하여 모래사장과 나뭇잎의 거친 노이즈 뭉침 현상을 제거했습니다. 아날로그 NTSC 컴포지트 수평 크로마 번짐 지연(`SCREEN_PIXEL_SIZE.x * 1.5`)과 은은한 3.5% CRT 애퍼처 그릴 마이크로 스캔라인을 추가했습니다.
  * **디더링 (Dithering):** 4x4 행렬에서 고정밀 8x8 베이어 행렬(64단계 임계값)로 업그레이드하여 윤곽선 밴딩 없는 부드러운 크로스해치 음영을 구현했습니다. 해상도 독립적 가상 레트로 그리드 스케일링(`SCREEN_PIXEL_SIZE.y * 540.0`)을 적용하여 720p, 1080p, 1440p, 4K 디스플레이 전반에서 디더 도트 크기가 일관되게 유지되며, 휘도 가중치 변조를 통해 하이라이트 노이즈를 억제했습니다.
  * **폭염 1984 (Heatwave 1984):** 단일 픽셀 연산 대신 실제 다중 샘플 광학 할레이션 확산(화면 UV 기준 4방향 십자 방사 커널)을 도입하여 태양 실루엣과 강한 플레어 주변으로 온화한 호박색-붉은빛이 자연스럽게 번지도록 구현했습니다. 아날로그 필름 톤 숄더 압축 곡선($C / (1.0 + 0.22C)$)을 적용하여 하이라이트 클리핑 없이 태양의 표정과 구름의 볼륨감을 섬세하게 보존하고, 빈티지 아나모픽 렌즈 감쇠 및 유기적 셀룰로이드 그레인을 완성했습니다.
